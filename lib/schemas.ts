...
export const personalInfoSchema = z.object({
  ...
  profilePicture: z
    .any()
    .refine(
      (files) => (files?.length > 0 ? files?.[0]?.size <= 2000000 : true),
...
export const jobDetailsSchema = z
  ...
  .superRefine((data, ctx) => {
    if (data.department === "HR" || data.department === "Finance") {
      ...
      const dayOfWeek = selectedDate.getDay(); // 5 for Friday, 6 for Saturday
      if (dayOfWeek === 5 || dayOfWeek === 6) { // Restrict Friday and Saturday
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            "Start Date cannot be on a Friday or Saturday for HR or Finance departments.",
          path: ["startDate"],
        });
      }
    }
...
export const skillsPreferencesSchema = z.object({
  ...
  skillExperiences: z.record(z.string(), z.number().min(0)).optional(), // New field for skill experiences
  ...
  extraNotes: z
    .string()
    .max(500, "Extra Notes cannot exceed 500 characters")
    .optional(),
